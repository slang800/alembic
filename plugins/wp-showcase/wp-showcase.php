...
class WordpressShowcase {
    ...
    function custom_columns( $column ) {
        global $post;
        if ($post->post_type != 'showcase_gallery') return;
        switch ( $column )
        {
            case 'images':     
                $limit = 5;
    ...
    function setting_image_sources() {
        ...
        if($html == '') $html = 'No connected image sources';
        echo $html;
    }
    ...
    function remove_showcase_menu() {
        ...
        if(isset($options['custom-roles'])) {
            ...
            if ($custom_roles) {
                ...
                if (!$show) remove_menu_page( 'edit.php?post_type=showcase_gallery' );
            }
        }
    }
    ...
    function meta_box_settings() {
        ...
        <table class="form-table"> 
            <tr valign="top">
                ...
                <span class="description"><?php _e('Choose an image source', 'showcase'); ?></span></td>
            </tr>
           <div id="new-image-sources">
            ...
            <tr valign="top" class="showcase_non_manual">
                <th scope="row">- <?php _e('Number of Images', 'showcase'); ?></th>
                <td><input type="text" name="showcase_settings[number_images]" value="<?php echo $this->default_val($options, 'number_images', ''); ?>" class="regular-text" /><br />
                <span class="description"><?php _e('The number of images to use in the gallery. Leave blank for all images. External sources default to 20.', 'showcase'); ?></span></td>
            </tr>
            <tr valign="top">
        ...
        if( $attachments ){
            ...
            if ($options['gallery_layout'] != 'slider') {
                ...
                if($layout == 'layout-full-data'){
                    ...
                    if(isset($meta['image_meta']['created_timestamp']) && $meta['image_meta']['created_timestamp']) $output .= '<p class="date-taken"><span>'. __('Date Taken:', 'showcase') .'</span> '. date('d M Y', $meta['image_meta']['created_timestamp']) .'</p>';
                    if(isset($meta['image_meta']['camera']) && $meta['image_meta']['camera'])                       $output .= '<p class="camera"><span>'. __('Camera:', 'showcase') .'</span> '. $meta['image_meta']['camera'] .'</p>';
                    if(isset($meta['image_meta']['focal_length']) && $meta['image_meta']['focal_length'])           $output .= '<p class="focal-length"><span>'. __('Focal Length:', 'showcase') .'</span> '. $meta['image_meta']['focal_length'] .'mm</p>';
                    if(isset($meta['image_meta']['aperture']) && $meta['image_meta']['aperture'])                   $output .= '<p class="aperture"><span>'. __('Aperture:', 'showcase') .'</span> f/'. $meta['image_meta']['aperture'] .'</p>';
                    if(isset($meta['image_meta']['iso']) && $meta['image_meta']['iso'])                             $output .= '<p class="iso"><span>'. __('ISO:', 'showcase') .'</span> ' . $meta['image_meta']['iso'] .'</p>';
                    if(isset($meta['image_meta']['shutter_speed']) && $meta['image_meta']['shutter_speed'])         $output .= '<p class="shutter-speed"><span>'. __('Shutter Speed:', 'showcase') .'</span> '. $pshutter .'</p>';
                    $output .= '</div>';
    ...
    function get_gallery_images( $post_id, $limit = -1 ) {
        ...
        if (!array_key_exists($options['source'], $sources)) $image_source = $this->image_source_default();
        else $image_source = $this->default_val($options, 'source', $this->image_source_default());
